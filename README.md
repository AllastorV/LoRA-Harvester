...
## LoRA-Harvester v5 · Kohya + AI Toolkit
**Dataset hazırlığından iki ayrı eğitim motoruna. Tek masaüstü çalışma alanı.**

[Kullanım ve kurulum](AI_TOOLKIT_NOTLARI.md) · [AI kontrolü](AI_KONTROL_KILAVUZU.md)

</div>

| Training / Eğitim | v5 ile eklenenler |
|---|---|
| **Motor seçimi** | Kohya korunur; Ostris AI Toolkit ikinci seçenek olarak açılır. |
| **Native YAML** | SD1.5, SDXL/Illustrious ve FLUX.1-dev görsel LoRA profilleri; step tabanlı ayarlar. |
| **Ayrı ortam** | Toolkit kendi Python/venv'sinde çalışır. Harvester paketleri yükseltilmez. |
| **Veri bütünlüğü** | Caption/master tag indeksleri, tekil dosya listesi, repeat desteği ve eski önizleme kontrolü. |
| **İş takibi** | Canlı günlük/step, adım sınırında durdurma, kontrollü resume ve çıktı kilidi. |

**Başlangıç:** `run.bat` → **Training → Eğitim motoru → AI Toolkit**. Toolkit bağlantısı sekmesinde ayrı kurulumun klasörünü ve Python dosyasını seç. `install.bat` Harvester içindir; Toolkit'i otomatik kurmaz. Ayrıntılar [AI_TOOLKIT_NOTLARI.md](AI_TOOLKIT_NOTLARI.md) içinde.

**Harvester kurulumu:** `install.bat` tek giriş noktasıdır. Çalışan NVIDIA GPU varsa CUDA 12.4, yoksa CPU PyTorch kurulur. Temel program, upscale, anime ve yüz paketleri kurulur; Ollama modeli ve ayrı AI Toolkit kurulumu buna dahil değildir. İşlem bittiğinde `run.bat` ile açın.

**Son doğrulama (2026-09-23):** Kurulum testlerinden 25'i geçti; Windows/Qt Studio arayüzü açıldı ve 6 görsellik veri seti GUI üzerinden tarandı. RTX 3070 Ti ile 238 görselin SFW/NSFW taraması çalıştı. Temiz sistem kurulumu, CPU yolu, GPU eğitimi ve görsel kıyafet tanıma doğrulanmadı. Aşağıdaki sürüm çağrıları önceki teslimlerin tarihsel kayıtlarıdır.

---

> **Güncel teslim — Canlı AI kontrolü (`2026.09.22-ai1`)**\
> Genel performans sürümü üzerine Codex/Claude için adlandırılmış canlı kontrol, oturumluk izin/onay paneli ve kurtarılabilir AI caption kaydı eklendi.\
> **Kullanım:** [AI_KONTROL_KILAVUZU.md](AI_KONTROL_KILAVUZU.md). Açılış `run.bat`; panel **Ctrl+Shift+A**.\
> **621 test geçti, 35 Qt testi atlandı.** Windows/Qt ve gerçek Codex/Claude oturumu doğrulanmadı. Aşağıdaki sürüm kayıtları önceki teslimlerdir.

> **Studio masaüstü arayüzü — r4:** Güncel kurulum ve kullanım için [STUDIO_ARAYUZ_NOTLARI.md](STUDIO_ARAYUZ_NOTLARI.md). Açılış: `run.bat`; klasik görünüm: `run_classic.bat`.

> **Studio geliştirme paketi — 2026.09.22-r2**\
> Tek kurulum/onarım sihirbazı, caption tabanlı veri dengesi, kullanıcı düzeltme hafızası ve yerel checkpoint karşılaştırması eklendi.\
> **Kurulum/onarım: `install.bat` · Açılış: `run.bat`.** Tek kurulum dosyası `install.bat` dosyasıdır. Yeni kurulumda arayüz ve sihirbaz İngilizce açılır; kayıtlı dil tercihi korunur.\
> Güncelleme ve kullanım: [GELISTIRME_KILAVUZU.md](GELISTIRME_KILAVUZU.md). **285 test geçti; 14 Qt testi atlandı.** Windows/GPU/gerçek model doğruluğu doğrulanmadı.\
> Aşağıdaki önceki v4.0 tanıtım/performans iddiaları bu teslimde yeniden ölçülmedi.
