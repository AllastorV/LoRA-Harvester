...
class OptimizedVideoProcessor:
    ...
    def __init__(self, 
        ...
        Args:
            batch_size: Number of frames to process in parallel (GPU) - default 8 for RTX 3070 Ti
        """
    ...
    def _process_batch(self, frames: List[np.ndarray], frame_numbers: List[int], 
        ...
        """
        Process a batch of frames with TRUE GPU batch processing
        """
        ...
        # GPU BATCH DETECTION - Process all frames at once!
        if hasattr(self.detector, 'detect_batch'):
            all_detections = self.detector.detect_batch(valid_frames)
        else:
            # Fallback to single detection
            all_detections = [self.detector.detect(f) for f in valid_frames]
        
        # Process detection results
        for idx, (frame, detections) in enumerate(zip(valid_frames, all_detections)):
            frame_num = frame_numbers[valid_indices[idx]]
