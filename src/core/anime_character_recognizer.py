...
# Fused embedding dimension: 512-d ResNet-18 + 96-d color histogram = 608-d
_EMBED_DIM = 608


class _AnimeCache:
    # v2: color histogram fused embedding — different from v1 resnet18-only
    _MODEL_KEY = "anime_resnet18_fused_v2"
    ...
    def get(self, path: Path) -> Optional[np.ndarray]:
        """Return (N, _EMBED_DIM) float32 or None on cache miss."""
        try:
            ...
            # Dimension-agnostic reshape: derive embedding dim from blob size.
            # Guards against stale-dimension cache entries from older versions.
            arr = np.frombuffer(blob, dtype=np.float32)
            if arr.size % n_faces != 0:
                return None  # corrupt / dimension mismatch → treat as miss
            arr = arr.reshape(n_faces, -1).copy()
            if arr.shape[1] != _EMBED_DIM:
                return None  # wrong dimension → re-extract
            return arr
        except Exception:
            return None

    def put(self, path: Path, embeddings: np.ndarray):
        """Store (N, _EMBED_DIM) array. Call with empty array for 0-face result."""
        try:
            stat = os.stat(str(path))
...
class AnimeCharacterRecognizer:
    ...
    def _detect_faces(self, image: np.ndarray) -> List[Tuple[int, int, int, int]]:
        ...
        Tries imgutils deepghs YOLO detector first; falls back to Haar cascade.
        """
        # Try imgutils (deepghs YOLO-based anime face detection — better recall)
        try:
            from imgutils.detect import detect_faces as _imgutils_faces
            from PIL import Image as _PIL
            rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            pil_img = _PIL.fromarray(rgb)
            raw = _imgutils_faces(pil_img, conf_threshold=0.4)
            # raw: list of ((x1,y1,x2,y2), label, score)
            results = []
            for (x1, y1, x2, y2), _label, _score in raw:
        ...
        # Fallback: Haar cascade (original)
    ...
    def _color_histogram(
        ...
        """
        Build a color histogram that captures character appearance.

        Three 32-bin HSV H-channel histograms:
          - Top 30% of face crop → hair color
          - Middle 40% of face crop → eyes + skin
          - Body region (if provided) OR bottom 40% of face → clothing
        Returns (96,) float32 L2-normalized.
        """
        h_img, w_img = face_crop.shape[:2]
        hsv = cv2.cvtColor(face_crop, cv2.COLOR_BGR2HSV)

        top_h = max(4, int(h_img * 0.30))
        mid_y1 = top_h
        mid_y2 = min(h_img, top_h + int(h_img * 0.40))

        regions = [
            hsv[:top_h, :],           # hair
            hsv[mid_y1:mid_y2, :],    # eyes/skin
        ]
        if body_crop is not None and body_crop.size > 0:
            regions.append(cv2.cvtColor(body_crop, cv2.COLOR_BGR2HSV))
        else:
            bot_y = max(mid_y2, int(h_img * 0.60))
            regions.append(hsv[bot_y:, :])   # lower face / clothing
        ...
        for region in regions:
            ...
            parts.append(hist / norm if norm > 1e-8 else hist)

        return np.concatenate(parts)

    def _embed_crop(self, image: np.ndarray, bbox: Tuple[int,int,int,int]) -> Optional[np.ndarray]:
        """
        Return fused embedding: [ResNet-18 (512-d)] + [color hist (96-d)] = 608-d.
        Both parts are L2-normalised individually before concat.
        """
        x, y, w, h = bbox
        m = int(max(w, h) * 0.15)
        ...
        # Color histogram (hair + eyes/skin + clothing)
        # Optionally extend body region below face bbox for clothing
        body_y2 = min(image.shape[0], y + h + int(h * 1.5))
        body_crop = image[y + h:body_y2, x1:x2] if (y + h) < image.shape[0] else None
        color_feat = self._color_histogram(crop, body_crop)

        return np.concatenate([emb_norm, color_feat])   # (608,)

    def _get_embeddings(self, img_path: Path) -> np.ndarray:
        """Return (N, 608) float32 array.  N=0 means no face detected."""
        ...
        result = (np.array(embs, dtype=np.float32)
                  if embs else np.empty((0, _EMBED_DIM), dtype=np.float32))
    ...
    def _cluster(self, embeddings: np.ndarray) -> np.ndarray:
        """
        Cluster embeddings. Tries HDBSCAN first (better for varied density),
        falls back to DBSCAN (scikit-learn, always available).

        Returns label array (-1 = noise / unassigned).
        """
        if len(embeddings) < self.cluster_min_samples:
            return np.full(len(embeddings), -1, dtype=int)

        # Try HDBSCAN (optional dep — much better than DBSCAN for anime)
        try:
            import hdbscan as _hdbscan
            clusterer = _hdbscan.HDBSCAN(
                min_cluster_size=max(2, self.cluster_min_samples),
                min_samples=1,
