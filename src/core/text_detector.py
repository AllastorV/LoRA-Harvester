...
class SubtitleDetector:
    ...
    def quick_text_check(self, frame: np.ndarray) -> bool:
        ...
        Optimized: downsample before processing for speed
        ...
        # Downsample for speed (max 640px width)
        sub_h, sub_w = subtitle_region.shape[:2]
        if sub_w > 640:
            scale = 640 / sub_w
            subtitle_region = cv2.resize(subtitle_region, (640, int(sub_h * scale)), interpolation=cv2.INTER_AREA)
            sub_w = 640
        ...
        for contour in contours:
            ...
            # - Reasonable size (scaled for downsampled image)
            # - In subtitle zone (bottom half)
            if (aspect_ratio > 2.5 and 
                w > sub_w * 0.08 and 
                area > 50 and  # Lowered for downsampled
