...
class ObjectDetector:
    ...
    def __init__(self, model_size: str = 'yolov8n.pt', confidence: float = 0.5):
        ...
        # Load YOLO model — always resolve to models/yolo/ to avoid duplicates
        import os, shutil, tempfile
        from src.core.model_paths import yolo_model_path
        _path = yolo_model_path(model_size)
        if _path.exists():
            self.model = _YOLO(str(_path))
        else:
            # Redirect ultralytics download into a temp dir, then move to models/yolo/
            with tempfile.TemporaryDirectory() as _tmp:
                ...
                try:
                    ...
                    os.chdir(_old_cwd)
                # Locate downloaded file and move to canonical location
                for _candidate in [
                    __import__('pathlib').Path(_tmp) / model_size,
                    __import__('pathlib').Path.home() / '.ultralytics' / 'assets' / model_size,
                    __import__('pathlib').Path(model_size),
                ]:
                    if _candidate.exists():
                        shutil.copy2(str(_candidate), str(_path))
                        break
    ...
    def detect_batch(self, frames: List[np.ndarray]) -> List[Dict[str, List[Dict]]]:
        """
        Detect objects in multiple frames at once (GPU batch processing)
        ...
        # Run batch inference - YOLO supports batch natively
        results_list = self.model(frames, conf=self.confidence, verbose=False)
        
        return [self._process_results(results) for results in results_list]
    
    def _process_results(self, results) -> Dict[str, List[Dict]]:
        """Process YOLO results into detection dictionary"""
        detections = {
            'person': [],
            'animal': [],
            'object': []
