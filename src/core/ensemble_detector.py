...
class EnsembleDetector:
    ...
    def detect(self, frame: np.ndarray) -> Dict[str, List[Dict]]:
        ...
        return self._categorize_detections(consensus_detections)
    
    def detect_batch(self, frames: List[np.ndarray]) -> List[Dict[str, List[Dict]]]:
        """
        Run ensemble detection on multiple frames (batch processing)
        Note: YOLO supports native batch, others run sequentially
        ...
        yolo_batch_results = []
        if 'yolo' in self.models:
            yolo_results = self.models['yolo'](frames, conf=self.confidence_threshold, verbose=False)
            for results in yolo_results:
                ...
                yolo_batch_results.append(detections)
        else:
            yolo_batch_results = [[] for _ in frames]
        
        # Process each frame with ensemble
        all_results = []
        for i, frame in enumerate(frames):
            all_detections = yolo_batch_results[i].copy()
            
            # Add DETR and Faster R-CNN (not batched yet)
            if 'detr' in self.models:
                all_detections.extend(self.detect_detr(frame))
    ...
    def _categorize_detections(self, consensus_detections: List[Detection]) -> Dict[str, List[Dict]]:
