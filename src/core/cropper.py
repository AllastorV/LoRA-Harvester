...
class SmartCropper:
    ...
    def _apply_overlay_exclusion(
        ...
        for ez in excluded_zones:
            ...
            # Step 8: shifting failed (e.g. crop spans full frame height and
            # the zone is a bottom subtitle band) → shrink the crop so the
            # zone is cut off, keeping the largest remaining side.
            cx2 = crop_x + crop_w
            cy2 = crop_y + crop_h
            if not (cx2 <= ez_x1 or crop_x >= ez_x2 or
                    cy2 <= ez_y1 or crop_y >= ez_y2):
                shrunk = self._shrink_crop_around_zone(
                    crop_x, crop_y, crop_w, crop_h,
                    ez_x1, ez_y1, ez_x2, ez_y2,
                )
                if shrunk is None:
                    return None
                crop_x, crop_y, crop_w, crop_h = shrunk

        return (crop_x, crop_y, crop_w, crop_h)

    def _shrink_crop_around_zone(
        self,
        crop_x: int, crop_y: int, crop_w: int, crop_h: int,
        ez_x1: int, ez_y1: int, ez_x2: int, ez_y2: int,
    ) -> Optional[Tuple[int, int, int, int]]:
        """
        Cut the crop on one side of the zone (the side keeping the most
        area), then re-fit the aspect ratio inside the kept region.
        Returns None if the zone covers essentially the whole crop.
        """
        cx2 = crop_x + crop_w
        cy2 = crop_y + crop_h

        candidates = []   # (side, kept_extent)
        if ez_y1 > crop_y:
            candidates.append(('bottom', ez_y1 - crop_y))   # keep top part
        if cy2 > ez_y2:
            candidates.append(('top', cy2 - ez_y2))         # keep bottom part
        if ez_x1 > crop_x:
            candidates.append(('right', ez_x1 - crop_x))    # keep left part
        if cx2 > ez_x2:
            candidates.append(('left', cx2 - ez_x2))        # keep right part

        if not candidates:
            return None

        side, kept = max(candidates, key=lambda c: c[1])

        old_cx = crop_x + crop_w // 2
        old_cy = crop_y + crop_h // 2
        ...
        # Re-fit aspect ratio inside the kept area
        if crop_h <= 0 or crop_w <= 0:
            return None
        if crop_w / crop_h > self.aspect_ratio:
            new_w = max(1, int(crop_h * self.aspect_ratio))
            # re-center horizontally around the original crop center
            crop_x = max(crop_x, min(old_cx - new_w // 2,
                                     crop_x + crop_w - new_w))
            crop_w = new_w
        else:
            new_h = max(1, int(crop_w / self.aspect_ratio))
            crop_y = max(crop_y, min(old_cy - new_h // 2,
                                     crop_y + crop_h - new_h))
            crop_h = new_h
