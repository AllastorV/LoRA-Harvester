...
if /i "%quality%"=="y" (
    ...
    %PYTHON_CMD% scripts\cli.py *.mp4 -f %format% -i %interval% --ensemble --turbo
) else (
    echo Running in STANDARD mode...
    %PYTHON_CMD% scripts\cli.py *.mp4 -f %format% -i %interval% --turbo
)
goto end
...
if /i "%quality%"=="y" (
    echo Running in HIGH QUALITY mode...
    %PYTHON_CMD% scripts\cli.py videos/*.mp4 -f %format% -i %interval% --ensemble --turbo
) else (
    echo Running in STANDARD mode...
    %PYTHON_CMD% scripts\cli.py videos/*.mp4 -f %format% -i %interval% --turbo
)
goto end
...
if /i "%quality%"=="y" (
    echo Running in HIGH QUALITY mode...
    %PYTHON_CMD% scripts\cli.py "%folder%"/*.mp4 -f %format% -i %interval% -c %confidence% --ensemble --turbo
) else (
    echo Running in STANDARD mode...
    %PYTHON_CMD% scripts\cli.py "%folder%"/*.mp4 -f %format% -i %interval% -c %confidence% --turbo
)
goto end
